...
public class MarkFacadeImplTest {
	...
	public void createListOfMarkDetailsFromSubjectName() throws AdaptorException, PersistenceServiceException {
		...
		results.add(new MarkDetailResult(Institute.ELTE, new Date(), 0));
		...
		final MarkDetailStub uniobudaStub = Mockito.mock(MarkDetailStub.class);
		stubs.add(uniobudaStub);
		final int yearBME = 2014;
		final double averageGradeBME = 2.4142;
		stubs.add(new MarkDetailStub(Institute.BME.toString(), yearBME, averageGradeBME));
		...
		Assert.assertEquals(markDetailStubs.get(0), uniobudaStub);
		Assert.assertEquals(markDetailStubs.get(1).getInstitute(), Institute.BME.toString());
		Assert.assertEquals(markDetailStubs.get(1).getYear(), yearBME);
		Assert.assertEquals(markDetailStubs.get(1).getAverageGrade(), averageGradeBME);
	}
